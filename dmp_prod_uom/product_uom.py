...
class product_template(osv.osv):
	...
	_columns = {
        'uom_id': fields.many2one('product.uom', 'Unit of Measure', required=True, 
								write=['dmp_product.group_product_manager'], read=['base.group_user'], 
...
limit 1
		...
		sql = 'select 1 as flag from account_analytic_line where product_id=%s \
				...
				limit 1'				
		for id in ids:
			#id_params = [id for i in range(21)]
			id_params = [id for i in range(18)]
	...
	def onchange_measure_type(self, cr, uid, ids, default_code, measure_type, context=None):
		...
		if measure_type == 'msp':
			...
			if len(uom_categ) > 0:
				...
				mod_name, uom_categ_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'dmp_prod_uom','uom_categ_msp_dummy')
				mod_name, uom_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'dmp_prod_uom','uom_msp_dummy')
	...
	def update_msp_uom_categ(self, cr, uid, product_id, vals, context=None):
		...
		if product.measure_type == 'msp':
			mod_name, categ_dummy_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'dmp_prod_uom','uom_categ_msp_dummy')
			mod_name, dummy_uom_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'dmp_prod_uom','uom_msp_dummy')
...
#	}    
	...
	def has_related_data(self, cr, uid, ids, context=None):
		...
		for id in ids:
			...
			id_params = [id for i in range(27)]
			cr.execute(sql, id_params)
			res = cr.fetchone()
			found_id = res and res[0] or False
			if found_id:
