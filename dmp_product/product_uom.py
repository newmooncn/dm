	...
	def onchange_measure_type(self, cr, uid, ids, default_code, measure_type, context=None):
		...
		if measure_type == 'msp':
			...
			if len(uom_categ) > 0:
				...
				mod_name, uom_categ_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'dmp_product','uom_categ_msp_dummy')
				mod_name, uom_id = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'dmp_product','uom_msp_dummy')
